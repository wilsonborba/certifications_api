...
async def get_context_from_app(
    ...
    info(
        f"Fetching context for item: {item_name}, identification: {input_identification}"
    )
    ...
    if not saved_questions:
        error(
            f"No saved questions found for item: {item_name}, identification: {input_identification}"
        )
        return None
    ...
    if len(saved_questions) < amount_question:
        ...
        if not new_saved_questions:
            error(
                f"Failed to generate enough questions for {item_name}/{input_identification}"
            )
...
def save_complaint(complaint_text: str, question_id: str, user_uuid_id: str) -> dict:
