...
class QuizPDFManager(BaseQuizManager):
    def __init__(
        ...
        *,
        ...
        self.pdf_adapter = PdfAdapter(
            raw=raw, filename=filename, upload=upload, document_id=document_id
        )
    ...
    async def save_questions(
        ...
        **kwargs,
        ...
        questions_pdf_id = redis_adapter.k(
            settings.QUESTIONS_PREFIX, document_id, attempt_index
        )
        ...
        await redis_adapter.set(
            questions_pdf_id, saved_questions, ex=1800 + (amount_question * 60)
        )  # 30 min + amount of question == minutes expiration
    ...
    async def get_questions(
        self, redis_adapter: RedisAdapter, document_id: str
    ) -> list[dict]:
        questions = []
        for idx in (1, 2):
        ...
        for idx in (1, 2):
            ...
            for q in data:
                if isinstance(q, dict) and all(
                    q.get(wk) == wv for wk, wv in where.items()
                ):
                    q[key] = value
                    debug(
                        f"Updated question in Redis {k}: set {key}={value} where {where}"
                    )
                    updated += 1
                    if not update_many:
                        break
    ...
    async def generate_context(
        ...
        try:
            ai_injection_result = await self.pdf_adapter.check_ai_injection(
            ...
            if ai_status in ("SUSPECT", "MALICIOUS"):
                raise MalwareDetectedError(
                    f"Malware AI Injection detected in document (status: {ai_status})."
                )
        ...
        try:
            response = await self.pdf_adapter.ai_client(user_uuid_id).generate_text(
                prompt=prompt,
